...
interface BlogDetailClientProps {
  blog: Blog & { author?: { name: string; avatar_url?: string; bio?: string } };
  locale: string;
}

export default function BlogDetailClient({ blog: initialBlog, locale }: BlogDetailClientProps) {
  ...
  const sanitizedContent = useMemo(() => sanitizeBlogContent(blog.content), [blog]);
  ...
  useEffect(() => {
    const recordView = async () => {
      ...
      if (user) {
        ...
        if (!existing) {
          await supabase.from('blog_views').insert({ blog_id: blog.id, user_id: user.id });
          await supabase.rpc('increment_blog_views', { blog_id: blog.id });
          setBlog(prev => ({ ...prev, views: prev.views + 1 }));
        }
      } else {
        const viewed = localStorage.getItem(`blog_viewed_${blog.id}`);
        if (!viewed) {
          localStorage.setItem(`blog_viewed_${blog.id}`, '1');
          await supabase.rpc('increment_blog_views', { blog_id: blog.id });
          setBlog(prev => ({ ...prev, views: prev.views + 1 }));
        }
      }
  ...
  const handleLike = useCallback(async () => {
    ...
    if (liked) {
      ...
      if (!error) {
        setLiked(false);
        setBlog(prev => ({ ...prev, likes: Math.max(0, prev.likes - 1) }));
