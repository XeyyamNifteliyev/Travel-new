...
interface BlogListClientProps {
  blogs: Blog[];
  allTags: string[];
  topAuthors: { name: string; avatar_url?: string }[];
  locale: string;
}

export default function BlogListClient({ blogs, allTags, topAuthors, locale }: BlogListClientProps) {
  ...
  const filteredBlogs = useMemo(() => {
    ...
    if (filterTag) {
      result = result.filter((blog) =>
        blog.tags?.some((tag: string) => tag.toLowerCase() === filterTag.toLowerCase())
      );
    }

    return result;
  }, [blogs, debouncedSearch, filterTag]);

  const featuredBlog = blogs[0];
  const restBlogs = filteredBlogs === blogs ? blogs.slice(1) : filteredBlogs;
          ...
          <div className="lg:col-span-8">
            ...
            {filteredBlogs.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {(debouncedSearch || filterTag ? filteredBlogs : restBlogs).map((blog, i) => (
                  <BlogCard key={blog.id} blog={blog} index={i} locale={locale} />
                ))}
              </div>
            )}
          </div>
