...
export default function PlaceError({
  ...
  useEffect(() => {
    console.error('Place page error:', { digest: error.digest, msg: error.message });
  }, [error]);
    ...
    <main className="min-h-[40vh] flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <p className="text-txt-sec mb-4">
          {locale === 'en' ? 'Failed to load place data' : locale === 'ru' ? 'Не удалось загрузить данные места' : 'Məkan məlumatı yüklənə bilmədi'}
        </p>
        <div className="flex gap-3 justify-center">
          ...
          <Link href={`/${locale}`} className="px-4 py-2 border border-border rounded-lg text-sm font-medium hover:border-primary/40 transition-colors flex items-center gap-2">
