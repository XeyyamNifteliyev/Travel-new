  ...
  if (conversations.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-txt-sec font-medium">{t('noConversations')}</p>
        <p className="text-txt-muted text-sm mt-1">{t('noConversationsSub')}</p>
      </div>
    );
  }
        ...
        if (isConfirmingDelete) {
          return (
            <div key={conv.id} className="p-4 rounded-xl bg-red-500/10 border border-red-500/20">
              <p className="text-sm text-txt mb-3">{t('deleteConversationConfirm')}</p>
              ...
              <div className="flex-grow min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h3 className={`font-semibold truncate ${isActive ? 'text-primary' : 'text-txt group-hover/item:text-primary'} transition-colors`}>
                    {conv.other_user?.name || t('user')}
                    {isBlocked && (
                      <span className="ml-2 text-[10px] text-amber-400 font-normal">{t('blocked')}</span>
                    )}
                  </h3>
                  {conv.last_message && (
                ...
                {conv.last_message ? (
                  <p className="text-sm text-txt-sec truncate">
                    {conv.last_message.sender_id === conv.user_id && (
                      <span className="text-primary/70">{t('you')}</span>
                    )}
                    {conv.last_message.text}
                  </p>
                ) : (
                  <p className="text-xs text-txt-muted">{t('newConversation')}</p>
                )}
              </div>
            </div>
