-- TravelAZ: Seed 5 cities with open travel data (Paris, Tbilisi, Dubai, London, Rome)
-- Sources: Wikipedia (CC BY-SA), OpenStreetMap/Overpass (ODbL)
-- Depends on: 020_open_travel_data.sql, 024_countries_cca2.sql
...
INSERT INTO external_import_logs (source, entity_type, imported_count, skipped_count, status, metadata, started_at, finished_at)
VALUES
  ('openstreetmap', 'place', 20, 0, 'success', '{"city": "paris", "mode": "overpass", "limit": 20}'::jsonb, now(), now()),
  ('openstreetmap', 'place', 20, 0, 'success', '{"city": "tbilisi", "mode": "overpass", "limit": 20}'::jsonb, now(), now()),
  ('openstreetmap', 'place', 20, 0, 'success', '{"city": "dubai", "mode": "overpass", "limit": 20}'::jsonb, now(), now()),
  ('openstreetmap', 'place', 20, 0, 'success', '{"city": "london", "mode": "overpass", "limit": 20}'::jsonb, now(), now()),
  ('openstreetmap', 'place', 20, 0, 'success', '{"city": "rome", "mode": "overpass", "limit": 20}'::jsonb, now(), now());
